...
void HoverButton::setSound(QString pathHover, QString pathLeave, QString pathPress, QString pathRelease){
    //设置覆盖、按压、释放、离开音效
    soundHover = (pathHover != QString("") ? new QSoundEffect(this) : nullptr);
    if(soundHover) {
        soundHover->setSource(QUrl::fromLocalFile(pathHover));
    }

    soundPress = (pathPress != QString("") ? new QSoundEffect(this) : nullptr);
    if(soundPress) {
        soundPress->setSource(QUrl::fromLocalFile(pathPress));
    }

    soundRelease = (pathRelease != QString("") ? new QSoundEffect(this) : nullptr);
    if(soundRelease) {
        soundRelease->setSource(QUrl::fromLocalFile(pathRelease));
    }

    soundLeave = (pathLeave != QString("") ? new QSoundEffect(this) : nullptr);
    if(soundLeave) {
